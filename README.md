...
> - **Update 2:** Tried bumping `sentiment_decay_rate` from the default `0.05` to `0.10` — decay happens noticeably faster, which feels more realistic for financial news (news sentiment fades quickly). Keeping `0.08` as my personal default for now as a middle ground.
> - **Update 3:** Tested `influence_radius` at `3` vs the default `5` — smaller radius produces more localized opinion clusters, which is interesting for modeling echo chambers. May write this up properly at some point.
