...
> - **Update 2:** Tried bumping `sentiment_decay_rate` from the default (0.05) up to 0.15 — sentiment shifts feel more realistic for financial news cycles where stories go stale quickly. Keeping this value in my local config. Side effect: consensus forms faster, so may need to increase simulation steps to compensate.
> - **Update 3:** Tested `interaction_radius` values of 0.1, 0.2, and 0.3. At 0.3 the network becomes too well-connected and opinions homogenize almost immediately. Settling on 0.15 as a sweet spot for financial scenarios — enough local clustering to see echo-chamber effects without full convergence.
