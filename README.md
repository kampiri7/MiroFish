...
> - **Update 2:** Tried bumping `sentiment_decay_rate` from the default `0.05` to `0.08` — sentiment dissipates faster, which feels more realistic for financial news cycles where stories lose relevance quickly. Results look more plausible for short-term market sentiment windows. Keeping this in my local config.
> - **Update 3:** Also experimenting with `influence_radius: 3` (up from default `2`) — larger radius means agents are affected by more neighbors, which seems to produce smoother consensus curves rather than sharp polarization spikes. Still evaluating whether this is actually more accurate or just masking interesting dynamics.
