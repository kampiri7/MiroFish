...
> - **Update 2:** Tried bumping `sentiment_decay_rate` from the default `0.05` to `0.08` — noticeably faster sentiment dissipation across the network, which feels more realistic for financial news cycles where stories lose relevance quickly. Keeping this value in my local config.
> - **Update 3:** Also set `interaction_radius` from `0.3` to `0.25` — reduces how broadly each agent influences neighbors, which seems to cut down on unrealistic consensus forming too early in the simulation. Still evaluating whether this is actually better or just different.
