...
> - **Update 2:** Tried bumping `sentiment_decay_rate` from the default (0.05) to 0.12 — sentiment signals dissipate faster, which feels more realistic for fast-moving financial news cycles. Results look more reasonable; keeping this in my local config.
> - **Update 3:** Tried values above 0.20 for `sentiment_decay_rate` and the simulation basically flattens out too quickly — agents converge to neutral before any interesting emergent behavior appears. Seems like 0.10–0.15 is the sweet spot for financial news seeds.
