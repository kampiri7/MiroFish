...
> - **Update 2:** Tried bumping `sentiment_decay_rate` from the default `0.05` to `0.10` — decay feels more realistic for financial news cycles where sentiment fades faster. Results look promising; the polarization effect I noted earlier is less sticky, which matches intuition. Keeping this value in my local config for now.
> - **Update 3:** Tested `interaction_radius` values of 0.2, 0.3, and 0.5 (default is 0.3). At 0.5, clusters form much faster but tend to be fewer and larger — loses some nuance. Sticking with 0.3 for now but 0.25 might be worth a try for more fragmented sentiment landscapes.
